...
#
# 目的: push を待たずローカルで全エコシステム(npm / dockerfile / github-actions /
...
#       RENOVATE_IMAGE で digest pin を渡す。このプロジェクトは dev コンテナ + npm だが、
#       renovate-local は別途この公式イメージをホストの docker で直接動かす。
#
